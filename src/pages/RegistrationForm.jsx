...
const RegistrationForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    seatNumber: null,
    paymentMethod: 'on-trip', // 'card' or 'on-trip'
    paid: false
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.seatNumber) newErrors.seatNumber = 'Please select a seat';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create registration with signature stored as base64 in Firestore
      ...
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'linear-gradient(135deg, #E0F7FA 0%, #B2EBF2 100%)' }}>
        <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-4">
            Thank you for registering for {trip.title}. You will receive a confirmation email shortly with your signed waiver.
          </p>
          <div className="p-4 rounded-lg" style={{ backgroundColor: '#E0F7FA' }}>
            <p className="font-semibold" style={{ color: colors.primary.teal }}>Your Seat: #{formData.seatNumber}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #E0F7FA 0%, #B2EBF2 100%)' }}>
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Seat Selection */}
          <div className="bg-white rounded-lg shadow-lg p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Select Your Seat *</h2>
            <VehicleSeatingMap
              vehicleType={trip.vehicleLayout}
              registrations={registrations}
              onSeatClick={(seatNumber) => setFormData({ ...formData, seatNumber })}
              selectedSeat={formData.seatNumber}
            />
            {errors.seatNumber && (
              <p className="text-red-500 text-sm mt-2">{errors.seatNumber}</p>
            )}
            {formData.seatNumber && (
