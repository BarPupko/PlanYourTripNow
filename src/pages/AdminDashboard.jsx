...
const AdminDashboard = () => {
  ...
  // Google Rating Badge
  const [googleForm, setGoogleForm] = useState({ rating: '', reviewCount: '', reviewUrl: '' });
  ...
  const handleSaveGoogleRating = async () => {
    ...
    try {
      await updateSiteSettings({
        googleRating: googleForm.rating ? parseFloat(googleForm.rating) : null,
        googleReviewCount: googleForm.reviewCount ? parseInt(googleForm.reviewCount, 10) : null,
        googleReviewUrl: googleForm.reviewUrl.trim() || null,
      });
    } catch (e) { console.error(e); } finally { setGoogleSaving(false); }
  };
        ...
        {!loading && allTrips.length > 0 && (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500 mt-0.5">Shown in the "Letters from the road" section. Enter your current Google rating to display it with a review link.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Rating (e.g. 4.8)</label>
                <input
                  type="number" min="1" max="5" step="0.1"
                  value={googleForm.rating}
                  onChange={e => setGoogleForm(p => ({ ...p, rating: e.target.value }))}
                  placeholder="4.8"
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm focus:border-[#00BCD4] focus:outline-none bg-white"
                />
              </div>
              <div>
                ...
                <input
                  type="number" min="0"
                  value={googleForm.reviewCount}
                  onChange={e => setGoogleForm(p => ({ ...p, reviewCount: e.target.value }))}
                  placeholder="127"
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm focus:border-[#00BCD4] focus:outline-none bg-white"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Google review link</label>
                <input
                  type="url"
                  value={googleForm.reviewUrl}
                  onChange={e => setGoogleForm(p => ({ ...p, reviewUrl: e.target.value }))}
                  placeholder="https://g.page/r/…/review"
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm focus:border-[#00BCD4] focus:outline-none bg-white"
                />
              </div>
            </div>
            <p className="text-xs text-gray-400 mb-3">To get your review link: open Google Maps → search IVRITours → click "Write a review" → copy the URL from the browser.</p>
            <div className="flex justify-end">
              <button
                onClick={handleSaveGoogleRating}
                disabled={googleSaving}
                className="px-4 py-2 text-white text-sm font-semibold rounded-lg hover:opacity-90 disabled:opacity-50"
                style={{ backgroundColor: colors.primary.teal }}
              >
